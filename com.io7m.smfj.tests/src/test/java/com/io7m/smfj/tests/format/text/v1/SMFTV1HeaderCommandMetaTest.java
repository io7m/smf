...
import com.io7m.smfj.format.text.v1.SMFTV1HeaderCommandMeta;
...
public final class SMFTV1HeaderCommandMetaTest
{
  @Test
    ...
    throws Exception
  {
    ...
    final SMFTV1HeaderCommandMeta cmd =
      new SMFTV1HeaderCommandMeta(reader, header);
    ...
    final SMFTParsingStatus r =
      cmd.parse(events, List.of("meta", "100"));
    Assert.assertEquals(SUCCESS, r);

    final SMFHeader result = header.build();
    Assert.assertEquals(100L, result.metaCount());
  }
    ...
    throws Exception
  {
    ...
    {{
      events.onError(this.with(new Delegate<SMFErrorType>()
      {
        ...
        {
          return e.message().contains(SMFTV1HeaderCommandMeta.SYNTAX);
        }
      }));
    }};

    final SMFTParsingStatus r =
      cmd.parse(events, List.of("meta"));
    ...
    throws Exception
  {
    ...
    final SMFTParsingStatus r =
      cmd.parse(events, List.of("meta", "z"));
    Assert.assertEquals(FAILURE, r);
    ...
    throws Exception
  {
    ...
    final SMFTParsingStatus r =
      cmd.parse(events, List.of("meta", "3", "4"));
    Assert.assertEquals(FAILURE, r);
